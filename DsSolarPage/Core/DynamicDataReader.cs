...
public class DynamicDataReader
{
    ...
    public DynamicData ReadForStack(string deviceId, int stackNo)
    {
        ...
        data.PowerFactorTotal = ReadS16(30079 + offset) / 1000.0f;  // 예: 스케일 0.001 가정
        ...
        // 누적/오늘 발전량 (기존 그대로)
        data.AccumKwh = ReadU32(30090 + offset);  // 30090~91
        data.TodayWh = ReadU32(30092 + offset);  // 30092~93

        // Peak / Max : 30094~30097
        data.PeakPowerInstall = ReadU32(30094 + offset);  // 30094
        data.PeakPowerToday = ReadU32(30095 + offset);  // 30095
        data.MaxActivePower = ReadU32(30096 + offset);  // 30096~97

        // 발전량
        data.AccumKwh = ReadU32(30091 + offset);  // kWh
        data.TodayWh = ReadU32(30093 + offset);  // 오늘 Wh
